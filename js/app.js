...
const paginateCount = () => {
    if (!localStorage.getItem("paginate-count")) {
        localStorage.setItem("paginate-count", `24`);
    }

    if (localStorage.getItem("paginate-count")) {
        localStorage.setItem(
            "paginate-count",
            `${Number(localStorage.getItem("paginate-count")) + 12}`
        );
    }
};
...
const filterProductsByFilterInfo = (products, filter) => {
    ...
    if (Object.keys(filter)) {
        ...
        if (filter.colors.length) {
            filteredProducts = products.filter((product) => {
                return product.color === filter.colors[0];
            });
        }

        // Category, Min price
        if (filter.category && filter.category !== "All" && filter.price.min) {
            filteredProducts = products.filter((product) => {
                return (
        ...
        // Category, Max price
        if (filter.category && filter.category !== "All" && filter.price.max) {
            filteredProducts = products.filter((product) => {
                return (
                    product.categories.includes(filter.category) &&
        ...
        // Category, Min price, Max price, Colors
        if (
            filter.category &&
            filter.category !== "All" &&
            filter.price.min &&
...
if (document.getElementsByClassName("colors__item")[1]) {
